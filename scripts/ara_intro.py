"""
Module: ara_intro — Contents and Introduction
...
def _find_placeholders(content: str) -> list[str]:
    """Return deduplicated, ordered list of placeholder names in content."""
    seen, order = set(), []
...
def _compute_values(
    ...
    derived: dict = {
        # AREA_COVERED_FULL = geographic location only (city, state, country)
        # matches 1_intro_poc.py reference behaviour
...
# child are preserved.
#
# Same logic as `scripts/ara_title.py:_merge_split_placeholders`. Kept in
# both modules for now; can be lifted into `core/` later.
# ─────────────────────────────────────────────────────────────────────────────
